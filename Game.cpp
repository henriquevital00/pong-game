...
,mPaddleDir(0)//direcao da bolinhamPaddleDir
,mPaddleDir2(0)//direcao da bolinhamPaddleDir
{
...
bool Game::Initialize()
{
	...
	mPaddlePos2.x = 1000.0f;//posição inicial da raquete eixo x
...
void Game::UpdateGame()
{
	...
	if (
		...
		diff2 <= paddleH / 2.0f &&
		// Estamos na posicao x correta
		mBallPos.x <= 990ssss.0f && mBallPos.x >= 985.0f &&
		// A bolinha está se movendo para a esquerda
		mBallVel.x > 0.0f)
	{
		mBallVel.x *= -1.0f;
