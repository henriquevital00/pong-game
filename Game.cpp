...
bool Game::Initialize()
{
	...
	mPaddlePos.x = 10.0f;//posição inicial da raquete eixo xw
	mPaddlePos.y = 768.0f/2.0f;//posição inicial da raquee eixo y

	sPaddlePos.x = 1000.0f;//posição inicial da raquete eixo x
	sPaddlePos.y = 768.0f / 2.0f;//posição inicial da raquee eixo y
...
void Game::UpdateGame()
{
	...
	sPaddlePos.y = mBallPos.y + rand() % 20 - 10;

	// verifique que a raquete não se move para fora da tela - usamos "thickness", que definimos como a altura dos elementos
	if (sPaddlePos.y < (paddleH / 2.0f + thickness))
	{
		sPaddlePos.y = paddleH / 2.0f + thickness;
	}
	else if (sPaddlePos.y > (768.0f - paddleH / 2.0f - thickness))
	{
		sPaddlePos.y = 768.0f - paddleH / 2.0f - thickness;
	}
	...
	float diff2 = sPaddlePos.y - mBallPos.y;
	...
	if (
		...
		diff2 <= paddleH / 2.0f &&
		// Estamos na posicao x correta
		mBallPos.x <= 995.0f && mBallPos.x >= 990.0f &&
		// A bolinha está se movendo para a esquerda
		mBallVel.x > 0.0f)
	{
		mBallVel.x *= -1.0f;
...
void Game::GenerateOutput()
{
	...
	SDL_Rect paddle2{
	static_cast<int>(sPaddlePos.x),//static_cast converte de float para inteiros, pois SDL_Rect trabalha com inteiros
	static_cast<int>(sPaddlePos.y - paddleH / 2),
	thickness,
	static_cast<int>(paddleH)
	};
